  ...
  <div class="flex min-h-screen">
    ...
    <main class="flex-1 p-6 bg-gray-100 overflow-y-auto">
      <div class="max-w-7xl mx-auto">
        <h1 class="text-3xl font-bold mb-6 text-gray-800 drop-shadow-sm">Quản lý phòng chiếu</h1>

        <!-- Button: Add Screening Room -->
        <button id="btnAddRoom" class="mb-6 bg-primary text-white px-5 py-3 rounded-xl shadow-md hover:shadow-primary/40 transition-all flex items-center gap-2">
          <i data-lucide="plus" class="w-5 h-5"></i> Thêm phòng chiếu
        </button>
        ...
        <div id="roomList" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <div class="bg-white p-5 rounded-2xl shadow hover:shadow-lg transition-all border border-gray-200">
            <h2 class="text-xl font-semibold text-gray-800">Phòng 1</h2>
            <p class="text-gray-600 mt-1">Số ghế: 120</p>
            <p class="text-gray-600">Loại: 2D</p>
            <div class="flex gap-3 mt-4">
              <button class="flex items-center gap-1 px-4 py-2 bg-blue-500 text-white rounded-lg hover:shadow-blue-300 shadow transition-all">
                <i data-lucide="edit" class="w-4 h-4"></i> Sửa
              </button>
              <button class="flex items-center gap-1 px-4 py-2 bg-red-500 text-white rounded-lg hover:shadow-red-300 shadow transition-all">
...
<!-- MODAL: Add/Edit Room -->
<div id="roomModal" class="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center opacity-0 pointer-events-none transition-all" >
  <div class="bg-white w-full max-w-md p-6 rounded-2xl shadow-2xl transform scale-90 transition-all" id="roomModalBox">
    <h2 id="modalTitle" class="text-2xl font-bold mb-4 text-gray-800">Thêm phòng chiếu</h2>

    <form id="roomForm" class="space-y-4">
      <input type="hidden" id="room_id">

      <div>
        <label class="font-medium">Tên phòng chiếu</label>
        <input id="room_name" type="text" class="w-full mt-1 p-3 border rounded-xl" required>
      </div>

      <div>
        <label class="font-medium">Số ghế</label>
        <input id="room_seats" type="number" class="w-full mt-1 p-3 border rounded-xl" required>
      </div>

      <div>
        <label class="font-medium">Loại phòng</label>
        <select id="room_type" class="w-full mt-1 p-3 border rounded-xl" required>
          <option>2D</option>
          <option>3D</option>
          <option>IMAX</option>
          <option>4DX</option>
      ...
      <div>
        <label class="font-medium">Gán suất chiếu</label>
        <select id="showtime_assign" class="w-full mt-1 p-3 border rounded-xl">
          <option value="">-- Chọn suất chiếu --</option>
        </select>
      </div>

      <div class="flex justify-end gap-3 pt-4">
        <button type="button" id="closeModal" class="px-4 py-2 bg-gray-300 rounded-xl">Hủy</button>
        <button type="submit" class="px-4 py-2 bg-primary text-white rounded-xl shadow hover:shadow-primary/40">Lưu</button>
      </div>
    </form>
  </div>
</div>
...
const box = document.getElementById('roomModalBox');
...
btnAdd.addEventListener('click', () => {
  document.getElementById('modalTitle').innerText = 'Thêm phòng chiếu';
...
// Load suat chieu
fetch('load_showtimes.php').then(r=>r.json()).then(list=>{
  let sel = document.getElementById('showtime_assign');
  list.forEach(s=>{
    sel.innerHTML += `<option value="${s.id}">${s.movie} - ${s.time}</option>`;
  });
});
