  ...
  <body class="bg-gray-50">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <?php
        ...
        // Lọc thể loại an toàn: chỉ cho phép 4 mã thể loại bạn dùng trong DB
        $allowed_genres = ['action', 'comedy', 'drama', 'horror'];
        $genreFilter = '';
        if (isset($_GET['genre']) && $_GET['genre'] !== '') {
            $g = $_GET['genre'];
            // kiểm tra whitelist để tránh SQL injection
            if (in_array($g, $allowed_genres, true)) {
                $genreFilter = $g;
            } else {
                // nếu giá trị không hợp lệ, bỏ qua (hiển thị tất cả)
                $genreFilter = '';
            }
        }
